...
command="restore $CUBX_ENV_BASE_RESTORE_FILENAME"

######################
# run
######################
docker run --name cubbles_base --rm $sourcesVolume $customConfigVolume -v "$CUBX_ENV_BASE_RESTORE_FOLDER:/backups" --volumes-from=$coreDataStoreContainer -v "/var/run/docker.sock:/var/run/docker.sock" $image $command
